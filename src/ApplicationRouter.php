...
final class ApplicationRouter implements iApplicationRouter
{
    ...
    /**
     * Nome da aplicação onde as rotas estão sendo
     * verificadas
     *
     * @var         string
     */
    ...
    /**
     * Caminho relativo para o arquivo de rotas
     * da aplicação.
     *
     * @var         string
     */
    ...
    /**
     * Array associativo contendo os valores padrões para 
     * as rotas de toda a aplicação
     *
     * @var         array
     */
    ...
    /**
     * Indica se o roteador pode executar a atualização 
     * do arquivo de rotas.
     *
     * @var         string
     */
        ...
        if (file_exists($appRoutes) === false) {
            ...
            if ($this->isUpdateRoutes === true) {
                ...
                foreach ($controllersFiles as $key => $fileName) {
                    if (!in_array($fileName, [".", ".."]) && !is_dir($fileName) && ends_with($fileName, ".php")) {
    ...
    /**
     * Cria uma instância de um objeto que implemente a interface
     * "iRouteConfig".
     *
     * @return      iRouteConfig
     */
    ...
    /**
     * Percorre todas as rotas definidas no controller indicado para
     * efetuar o registro de cada uma delas.
     *
    ...
    private function registerControllerRoutes(string $controllerName) : void
    {
        ...
        if (count($routesConfig) > 0) {
            ...
            foreach ($routesConfig as $cfg) {
                ...
                if (in_array_ci($method, $invalidMethods)) {
    ...
    /**
     * Retorna a mescla de 2 arrays contendo as configurações brutas para 
     * rotas;
     *
     ...
     *              Quando "true" não permitirá a mesclagem de
     *              todas as propriedades e sim apenas as permitidas
     *              para os controllers.
     * 
     * @return      array
     */
     ...
     * @return      ?array
     */
    public function selectTargetRawRoute(string $targetMethod, string $targetRoute) : ?array
    {
        ...
        if (count($appRoutes) !== 0) {
            ...
            if ($rawRoute === null && isset($appRoutes["complex"])) {
                $matchRoutes = [];
                $matchIndex = 0;
